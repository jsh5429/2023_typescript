// 타입스크립트는 자바스크립트의 변수와 함수에 타입을 설정한다.
console.log('안녕');

// 변수의 타입을 설정
// 변수를 지정할 때 타입을 지정하여 원하는 자료형만 넣을 수 있다.
let word : string = "안녕";
word = "반갑습니다.";
// 오류. 다른 자료형은 들어가지 않는다.
//word = 1;
...
// 매개변수의 자료형과 printWord의 자료형을 정의해라.
...
// 객체의 인터페이스 - 이름 작성 시 첫 글자 대문자
// age값은 들어와도 안들어와도 상관없다. 그러나 들어온다면 number로 들어와야한다.
interface Person {
    name : string,
    age? : number
}
...
let myPerson : Person = {
    name : "green",
    age : 40,
    //adress : "", // Person에서 작성하지 않은 내용은 객체에 쓸 수 없다.
}

console.log(myPerson.name)

// 함수 안의 매개변수의 객체를 인터페이스로 타입을 정한다.
// 매개변수의 객체의 인터페이스 타입은 함수 안에서 쓰는 속성
function printName(person:Person):void{
    console.log(person.name);
    // if문으 ㄹ사용해서 age값이 있다면 console로 출력
    if(person.age){
...
printName(person);
// pringName을 사용해서 myPerson 출력
printName(myPerson);

// 함수를 작성할 때 인터페이스로 먼저 알려주고 작성
interface login {
    (username : string, password : string) : boolean;
}

// 화살표함수, 익명함수를 만들어서 변수에 할당해서 쓴다.
...
// 자료형은 이미 위에서 정해졌기 때문에 따로 써주지않아도 된다.
// 그런데 없었던 매개변수를 추가하면 식이 달라졌기 때문에 다시 자료형을 다 써주어야한다.
loginUserInter = function(id, pw){
    // 매개변수의 자료형을 기록했기에 적지않아도 괜찮ㄴ다.
